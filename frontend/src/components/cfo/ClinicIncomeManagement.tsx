...
const ClinicIncomeManagement: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(editingId ? `/api/cfo/clinic-income/${editingId}` : '/api/cfo/clinic-income', {
      method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(recordData)
    });
    if (res.ok) { 
      showAINotification(editingId ? '✅ Income updated' : '✅ Clinic income recorded', 2000); 
  ...
  const onDelete = async (id: number) => {
    ...
    const res = await fetch(`/api/cfo/clinic-income/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 p-6">
      <div className="flex items-center justify-between mb-8 max-w-7xl mx-auto">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-700 to-indigo-700 bg-clip-text text-transparent">Clinic Income Management</h1>
          <p className="text-gray-600 mt-2">Record and manage clinic income.</p>
        </div>
        ...
        <div className="fixed inset-0 bg-gradient-to-br from-blue-900/20 via-purple-900/20 to-indigo-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/80 backdrop-blur-md border border-white/30 rounded-2xl w-full max-w-2xl shadow-2xl">
            <div className="px-6 py-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-t-2xl flex items-center justify-between">
              <h3 className="text-lg font-bold">Add Clinic Income</h3>
...
export default ClinicIncomeManagement;
