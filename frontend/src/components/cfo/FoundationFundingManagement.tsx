...
const FoundationFundingManagement: React.FC = () => {
  const { user } = useAuth();
  ...
  const [form, setForm] = useState({ fundType: '', source: '', amount: '', description: '', date: new Date().toISOString().slice(0,10) });
  ...
  const loadData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setRecords(data.foundationFunding || []);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch(editingId ? `/api/cfo/foundation-funding/${editingId}` : '/api/cfo/foundation-funding', {
      method: editingId ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...form, amount: Number(form.amount), recordedBy: user?.name || 'CFO' })
    });
    if (res.ok) { 
      showAINotification(editingId ? '✅ Funding updated' : '✅ Foundation funding recorded', 2000); 
      setShowModal(false); 
  ...
  const onEdit = (r: any) => {
    setForm({ fundType: r.fundType, source: r.source, amount: String(r.amount), description: r.description || '', date: String(r.date).slice(0,10) });
    setEditingId(r.id);
    setShowModal(true);
  };
  ...
  const onDelete = async (id: number) => {
    ...
    const res = await fetch(`/api/cfo/foundation-funding/${id}`, { method: 'DELETE' });
    if (res.ok) { showAINotification('🗑️ Deleted', 1500); loadData(); } else showAINotification('❌ Delete failed', 3000);
  };

  const toggleSource = (source: string) => {
    setOpenSources(prev => { const n = new Set(prev); if (n.has(source)) n.delete(source); else n.add(source); return n; });
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 p-6">
      <div className="flex items-center justify-between mb-8 max-w-7xl mx-auto">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-700 to-indigo-700 bg-clip-text text-transparent">Foundation Funding Management</h1>
          <p className="text-gray-600 mt-2">Manage foundation funding records.</p>
        </div>
        <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl shadow-lg hover:from-purple-700 hover:to-indigo-700 transition-all">Add Funding</button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gradient-to-br from-blue-900/20 via-purple-900/20 to-indigo-900/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/80 backdrop-blur-md border border-white/30 rounded-2xl w-full max-w-2xl shadow-2xl transition-transform hover:-translate-y-0.5">
            <div className="px-6 py-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-t-2xl flex items-center justify-between">
              <h3 className="text-lg font-bold">Add Foundation Funding</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.fundType} onChange={e => setForm({ ...form, fundType: e.target.value })} className="w-full rounded-lg border-gray-300 focus:ring-purple-500 focus:border-purple-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Amount (UGX)</label>
                  <input type="number" min="0" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} className="w-full rounded-lg border-gray-300 focus:ring-purple-500 focus:border-purple-500" />
                </div>
                <div>
...
export default FoundationFundingManagement;
