...
interface SuperTeacherSidebarProps {
  isCollapsed: boolean;
  setIsCollapsed: (collapsed: boolean) => void;
}

const SuperTeacherSidebar: React.FC<SuperTeacherSidebarProps> = ({ isCollapsed, setIsCollapsed }) => {
  const { shouldShowRestrictedAccess } = useRestrictedAccess();
  const superTeacherNavItems = [
    { path: '/dashboard', icon: Home, label: 'Dashboard' },
    { path: '/students', icon: Users, label: 'All My Students' },
        ...
        <div className="flex items-center space-x-3">
          ...
          {!isCollapsed && (
            <div>
              ...
              <p className="text-sm text-gray-500">Super Teacher Portal</p>
            </div>
          )}
        </div>
      </div>
...
export default SuperTeacherSidebar;
